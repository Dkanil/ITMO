...
def grad_f(x1, x2, eps):
    df1 = df(lambda x: func(x, x2), x1, eps)
    df2 = df(lambda x: func(x1, x), x2, eps)
...
def gradient_spusk(x, eps, alpha=0.1, find_min=True):
    ...
    while True:
        ...
        if np.linalg.norm(g) <= eps or abs(func(prev[0], prev[1]) - func(x[0], x[1])) <= eps:
            return x, func(x[0], x[1]), iters
        if ((find_min and func(x[0], x[1]) > func(prev[0], prev[1])) or
                (not find_min and func(x[0], x[1]) < func(prev[0], prev[1]))):
            x = prev
            alpha *= 0.5
...
def fastest_spusk(x, eps, find_min=True):
    ...
    while True:
        ...
        if find_min:
            s_k = -s_k
        phi = lambda h: func(x[0] + h * s_k[0],
                             x[1] + h * s_k[1])  # подставляем в функцию следующие координаты, которые зависят от h
