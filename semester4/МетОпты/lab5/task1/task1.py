...
history = [loss_function(params_start)]
def callback(current_params):
    history.append(loss_function(current_params))
...
for i in range(50):
    for j in range(50):
        Z_grid[i, j] = gauss_2d(X_grid[i, j], Y_grid[i, j], A_opt, x0_opt, y0_opt, sigma_x_opt, sigma_y_opt, theta_opt,
                                offset_opt)  # аргументы x, y для gauss_2d теперь берутся в узлах равномерной сетки
...
plt.ylabel("MSE")
plt.title("Кривая обучения (Loss) для Гауссианы")
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('task1_learning_curve.png', dpi=300)
plt.show()
plt.close()
