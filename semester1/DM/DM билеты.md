...
Пусть $A \sub V_1(G)$, тогда из вершин $A$ выходит не менее чем $k \cdot |A|$ рёбер к вершинам из $N_G(A)$, а в каждую вершину $b \in N_G(A)$ входит не более, чем $k$ рёбер из вершин множества $A$.
- Таким образом, 
...
- Пусть $M \sub V_1$, а $A_{i_1} , \dots , A_{i_m}$ — все юноши, чьи копии есть в $M$.
...
• По [Лемме 1](#лемма-1) и [Теореме Галлаи](#теорема-галлаи) мы имеем
$\begin{cases}
	\alpha(G) + \beta(G) = v(G) \\
	\alpha'(G) + \beta'(G) = v(G)
\end{cases}$
